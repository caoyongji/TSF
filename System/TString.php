...
class TString
{
    ...
    public static function makeGuid()
    {
        ...
        $secure_str = sprintf('%08s%04s%04x%04x%012s', $time_low, $time_mid, $time_hi_and_version, $clock_seq_hi_and_reserved, $node);
        return md5($secure_str.microtime(true));
    }
    ...
    //all the user_name of all the account types must be componented of number,charactor,or _
    public static function checkUserName($user_name)
    {
        $result = preg_match('/^[a-z,A-Z,_,0-9]+$/', $user_name);
        if(empty($result))
        {
            return false;
        }
